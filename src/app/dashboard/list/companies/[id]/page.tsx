...
import ManageFilesModal from "@/app/ui/files/manage-files-modal";
...
export default function SingleLocationPage({
  ...
  const canEdit =
    role === "admin" ||
    (role === "pharmacy_manager" && appUser?.companyId === company.id);
  const hasContactInfo =
    company?.contactName || company?.contactPhone || company?.contactEmail;

  const companyLogo = company?.files.find(
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="bg-primary py-6 px-4 rounded-md flex-1 flex gap-4 text-white">
            ...
            <div className="w-2/3 flex flex-col justify-between gap-4">
              ...
              <p className="text-sm">
                {company.approved
                  ? "Approved: This pharmacy can post shifts"
                  : "Not Approved: This pharmacy can't post shifts"}
              </p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col">
                    {hasContactInfo ? (
                      <>
                        <span>
                          {company?.contactName || "Add contact's name"}
                        </span>
                        <span>
                          {company?.contactPhone || "Add contact's phone"}
                        </span>
        ...
        <div className="bg-white p-4 rounded-md">
          ...
          <div className="mt-4 flex gap-4 flex-wrap text-xs text-white font-medium">
            <Link
              ...
              href={`/dashboard/shifts?companyId=${id}`}
            >
              Company&apos;s Shifts
            </Link>
            <Link
              className="p-3 rounded-md bg-complementary-one"
              href={`/dashboard/list/users?companyId=${id}`}
            >
              Company&apos;s Managers
            </Link>
            {/* <Link className="p-3 rounded-md bg-complementary-two" href="/">
              Company&apos;s Reports
            </Link>
            <Link className="p-3 rounded-md bg-secondary" href="/">
              Company&apos;s Files
            </Link>
            <Link className="p-3 rounded-md bg-complementary-one" href="/">
