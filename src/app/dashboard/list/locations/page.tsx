...
// TODO move location table to another page
type Location = {
    id: number,
    name: string,
    email?: string,
...
export default async function LocationsList({
  searchParams,
    ...
    const totalPagesCompanies=companiesResponse.meta?.totalPages;

    const locations = await fetchLocations();

    const totalPagesLocations = 4; //MODIFY 

    

    const renderRowCompany = (item: Company) => (
    <tr
      key={item.id}
  ...
  const renderRowLocation = (item: Location) => (
    <tr
      key={item.id}
      className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-purple-50"
    >
    ...
    <div className="p-4 lg:p-8">
        ...
        <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
            ...
            <div style={{overflowX: 'scroll'}}>
                <Table columns={columnsCompanies} renderRow={renderRowCompany} data={companies}/>
            </div>
            {/* PAGINATION */}
            <div className="mt-5 flex w-full justify-center">
                <Pagination totalPages={totalPagesCompanies} />
            </div>
        </div>
        <h1 className={`${lusitana.className} mb-4 text-xl md:text-2xl`}>
            Locations List
        ...
        <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
            ...
            {/* LIST */}
            <div style={{overflowX: 'scroll'}}>
                <Table columns={columnsLocations} renderRow={renderRowLocation} data={locations}/>
            </div>
            {/* PAGINATION */}
            <div className="mt-5 flex w-full justify-center">
                <Pagination totalPages={totalPagesLocations} />
            </div>
        </div>
    </div>
  );
