...
import { setManagerAllowedCompanies, setPharmacistAllowedCompanies } from "@/app/lib/actions";
...
export default function SelectManagedCompaniesForm({ 
  token,
  setOpen,
  data, 
  userId, 
  ...
  const [selectedOptions, setSelectedOptions] = useState<any[]>([]);
    ...
    const {
      ...
      const [state, formAction] = useFormState(
          setManagerAllowedCompanies.bind(null, token),
        {
          success: false,
          error: false,
        }
    ...
    useEffect(() => {
      if (state.success) {
        toast(`Managed Companies Saved!`, {toastId: 'unique-toast'});
        setOpen(false);
        window.location.reload();
      }
    }, [state, setOpen])

    //Read current allowed pharmacies from selected Manager
    useEffect(() => {
  ...
  const handleCheckboxChange = (event: { target: { value: any; checked: any; }; }) => {
    ...
    <div className='p-4 flex flex-col gap-4'>
      <h2 className="text-md font-semibold mb-2">Select Manager's Pharmacies:</h2>
      <input  
          value={userId} 
          defaultValue={userId} 
          {...register("id")}
      ...
      <span className="text-center font-medium">Manager will be able to view all the information from the selected pharmacies</span>
      {errors.id?.message && ( 
