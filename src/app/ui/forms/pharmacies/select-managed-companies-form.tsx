...
import {
  setManagerAllowedCompanies,
  setPharmacistAllowedCompanies,
...
export default function SelectManagedCompaniesForm({
  ...
  useEffect(() => {
    if (data?.allowedCompanies && data?.allowedCompanies.length > 0) {
      const companyIds = data.allowedCompanies.map(
    ...
    <form onSubmit={onSubmit}>
      <div className="p-4 flex flex-col gap-4">
        ...
        <span className="text-center font-medium">
          Manager will be able to view all the information from the selected
          pharmacies
        </span>
        {errors.id?.message && (
