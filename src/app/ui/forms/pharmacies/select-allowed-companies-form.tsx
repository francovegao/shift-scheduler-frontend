...
const FORM_CONFIG = {
  set_pharmacist_permissions: {
    ...
    description:
      "Select which pharmacies this pharmacist can access and toggle their ability to see pay rates for each.",
    footerNote:
      "The pharmacist will only see shifts and pay rates for the items checked above.",
    buttonText: "Save All Permissions",
  },
  set_allowed_companies: {
    title: "Select Allowed Pharmacies",
    description:
      "Select the pharmacies this pharmacist is authorized to see. Pay rate access is enabled by default for selected pharmacies.",
    footerNote:
      "Only shifts from the selected pharmacies will be visible to the pharmacist.",
    buttonText: "Save Allowed Pharmacies",
  },
  set_allowed_pay_rates: {
    title: "Manage Pay Rate Visibility",
    description:
      "Toggle whether the pharmacist can view pay rates for their assigned pharmacies.",
    footerNote:
      "Only pay rates from the selected pharmacies will be visible to the pharmacist.",
      ...
      try {
        ...
        if (type === "set_allowed_pay_rates") {
          const allAsSelected = fetchedCompanies.map((c: any) => {
            const existing = data?.companyPermissions?.find(
              (p: any) => p.companyId === c.id,
            );
            return {
              companyId: c.id,
                ...
                {isSelected && type !== "set_allowed_companies" && (
                  <div className="ml-7 mt-1 flex items-center gap-2 bg-gray-50 p-1 rounded">
                    <input
                      type="checkbox"
                      {...register(
                        `companyPermissions.${permissionIndex}.canViewPayRate`,
                      )}
                      className="h-4 w-4"
                    />
