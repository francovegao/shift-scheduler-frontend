...
export default function UserForm({
  ...
  const [state, formAction] = useFormState(
    type === "create"
      ? createUser.bind(null, token)
      : updateUser.bind(null, token),
    {
      success: false,
      error: false,
    },
    ...
    <form className="flex flex-col gap-8 text-foreground" onSubmit={onSubmit}>
      ...
      <div className="flex justify-between flex-wrap gap-4">
        ...
        {type === "create" ? (
          <div className="flex flex-col gap-2 w-full md:w-1/4">
            <label className="text-xs text-tx-body-muted">Role</label>
            <select
          ...
          <InputField
            ...
            inputProps={{
              disabled: true,
              className:
                "border border-surface-muted bg-surface text-tx-primary p-2 rounded-md text-sm w-full disabled:bg-surface-muted/50 disabled:text-tx-disabled disabled:cursor-not-allowedl",
            }}
          />
        )}
        <InputField
      ...
      <div className="flex justify-between flex-wrap gap-4">
        {/*<div className="flex flex-col gap-2 w-full md:w-1/4 justify-center">
              <label
                className="text-xs text-tx-body-muted flex items-center gap-2 cursor-pointer"
                htmlFor="resume"
              >
