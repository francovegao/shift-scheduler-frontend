...
export default function ManageFilesModal({
  ...
  const Form = () => {
    ...
    useEffect(() => {
      if (state.success) {
        toast(
          `${fileType === "profilePicture" ? "Profile picture" : fileType} has been deleted!`,
    ...
    return type === "delete" && id ? (
      <form className="p-4 flex flex-col gap-4" action={formAction}>
        ...
        <span className="text-center font-medium">
          ...
          {fileType === "profilePicture" ? "profile picture" : fileType}?
        </span>
        <span className="text-center font-medium">
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center">
          <div className="bg-surface text-foreground p-4 rounded-md relative w-[90%] md:w-[70%] lg:w-[60%] xl:w-[50%] 2xl:w-[40%]">
            <Form />
            <div
